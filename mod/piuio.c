...
#define COIN_HI      15            /* byte-15 only!             */
...
struct piuio {
	...
	u8  coin_last;                  /* ▲ now one byte only      */
...
static int probe(struct hid_device *hdev,const struct hid_device_id *id)
{
	...
	/* baseline for coin byte */
	if(read_coin_hi(p,&p->coin_last)) p->coin_last = 0;
...
MODULE_DESCRIPTION("PIUIO 0x1020 – 10-button game-pad");