 ...
 * --- Debugging Note ---
 * This version fixes -Wmisleading-indentation warnings.
 * Initialization: Wake ONCE, Initial coin state READ during probe.
...
struct piuio { /* ... same context struct ... */
	...
	u8 btn_buf[RPT_LEN_BTN]; bool down[NBTN]; u8 coin_last; u8 coin_buf[COIN_BUF_LEN];
...
// *** MODIFICATION: Fix -Wmisleading-indentation ***
static int get_matrix(struct piuio *p)
{
	...
	// Check return value with clearer structure
	if (r == RPT_LEN_BTN) {
		return 0; // Success
	} else if (r >= 0) { // Check for wrong length explicitly
		pr_warn_ratelimited("get_matrix: Incorrect length received (%d, expected %d)\n", r, RPT_LEN_BTN);
		return -EIO;
	} else { // Explicit else for negative error codes
...
static int read_coin_hi(struct piuio *p, u8 *hi)
{
	...
	// Check return value with clearer structure
	if (r >= COIN_OFFSET_HI + 1) { // Check for sufficient length first
		*hi = p->coin_buf[COIN_OFFSET_HI];
		return 0; // Success
	} else { // Handle insufficient length OR negative error code
		if (r >= 0) { // Explicit check for insufficient length (but non-negative r)
			pr_warn_ratelimited("read_coin_hi: Report too short (%d bytes, needed %d)\n", r, COIN_OFFSET_HI + 1);
			return -EIO;
		} else { // Explicit else for negative error codes
...
/* --- probe / remove --- */
static int probe(struct hid_device *hdev, const struct hid_device_id *id) { /* ... same as before ... */
	struct usb_interface *itf = to_usb_interface(hdev->dev.parent); struct piuio *p; int r = 0; int b;
	pr_info("Probe function entered for VID=0x%04X PID=0x%04X\n", id->vendor, id->product);
	if (ctx) { pr_err("Device context already exists.\n"); return -EBUSY; }
	p = devm_kzalloc(&hdev->dev, sizeof(*p), GFP_KERNEL); if (!p) { pr_err("Failed context allocation.\n"); return -ENOMEM; } pr_info("Device context allocated.\n");
	p->hdev = hdev; p->udev = interface_to_usbdev(itf); p->iface = itf->cur_altsetting->desc.bInterfaceNumber; p->hid_started = false; atomic_set(&p->stop, 0);
	hid_set_drvdata(hdev, p); pr_info("Context basic fields initialized.\n");
	pr_info("Parsing HID report descriptor...\n"); r = hid_parse(hdev); if (r) { pr_err("hid_parse failed! Error %d\n", r); return r; } pr_info("hid_parse OK.\n");
	pr_info("Starting HID hardware communication...\n"); r = hid_hw_start(hdev, HID_CONNECT_HIDRAW); if (r) { pr_err("hid_hw_start failed! Error %d\n", r); return r; } p->hid_started = true; pr_info("hid_hw_start OK.\n");
	pr_info("Sending SET_IDLE request...\n"); usb_control_msg(p->udev, usb_sndctrlpipe(p->udev, 0), HID_REQ_SET_IDLE, USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE, 0, p->iface, NULL, 0, 10);
	pr_info("Calling send_wake (Single attempt)...\n"); send_wake(p);
	pr_info("Allocating input device...\n"); p->idev = devm_input_allocate_device(&hdev->dev); if (!p->idev) { pr_err("Failed input alloc.\n"); r = -ENOMEM; goto err_hw; } pr_info("Input device allocated.\n");
	p->idev->name = "PIUIO Gamepad + Coin"; p->idev->phys = "piuio/input0"; p->idev->id.bustype = BUS_USB; p->idev->id.vendor = VID_PIUIO; p->idev->id.product = PID_PIUIO1020; p->idev->dev.parent = &hdev->dev;
	set_bit(EV_KEY, p->idev->evbit); for (b = 0; b < NBTN; b++) { set_bit((b < 5) ? map[b].p1 : map[b - 5].p2, p->idev->keybit); } set_bit(KEY_COIN, p->idev->keybit); pr_info("Input device properties configured.\n");
	pr_info("Registering input device...\n"); r = input_register_device(p->idev); if (r) { pr_err("Failed input register! Error %d\n", r); goto err_hw; } pr_info("Input device registered.\n");
	pr_info("Reading initial coin state post-wake...\n"); if (read_coin_hi(p, &p->coin_last)) { pr_warn("Failed initial coin read, default 0.\n"); p->coin_last = 0; } else { pr_info("Initial coin hi-byte: 0x%02X\n", p->coin_last); }
	pr_info("Initializing polling...\n"); INIT_WORK(&p->poll_wq, poll_work); hrtimer_init(&p->timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL); p->timer.function = timer_cb; hrtimer_start(&p->timer, ms_to_ktime(clamp_poll()), HRTIMER_MODE_REL);
	ctx = p; pr_info("Device attached successfully. Poll: %d ms.\n", clamp_poll()); return 0;
err_hw:
	pr_err("Probe failed during setup. Cleaning up...\n"); if (p->hid_started) hid_hw_stop(hdev); if (ctx == p) ctx = NULL; return r;
}
